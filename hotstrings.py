...
class Hotstrings:
    def __init__(self):
        ...
        self.user_input = deque(maxlen = 50)
    ...
    def create_tray_icon(self):
        ...
        self.normal_icon = str(self.cwd / 'normal_icon.png')
        if not Path(self.normal_icon).exists():
            # Generic default icon if custom icon not present
            self.normal_icon = QIcon(QApplication.style().standardIcon(QApplication.style().SP_ComputerIcon))
        self.paused_icon = str(self.cwd / 'paused_icon.png')
        if not Path(self.paused_icon).exists():
    ...
    def change_endchar(self):
        text, ok = QInputDialog.getText(None, 'Change endchar', 'Enter the new endchar:')
        self.restart = True
        if ok and text:
            if len(text) == 1:
                self.endchar = text
